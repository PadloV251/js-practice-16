...
function compareArrays(arr1, arr2) {
  ...
  const sortedArray = JSON.stringify(arr1) === JSON.stringify(arr2);
  console.assert(sortedArray, "Масиви не однакові!");
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
      ...
      stringMatrix.push(randomNumber);
    }
    arrMatrix.push(stringMatrix);
  }
...
function validateUserInput(userInput) {
  console.assert(userInput.hasOwnProperty('username'), "Помилка: ім'я користувача відсутнє!");
  console.assert(userInput.hasOwnProperty('password'), "Помилка: пароль відсутній!");
